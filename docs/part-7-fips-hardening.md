...
compliance target is a question this repo does not answer for you. One
further open item: whether the `clickhouse-server` image's own bundled
`wget` (used by an informational health-probe line, not the asserted check)
supports HTTPS/`--no-check-certificate` could not be confirmed without a live
`kubectl exec` against a running pod, which this cycle's constraints forbid;
it remains an open deferral (`d1` in the plan record) for a future
...
- [ ] `clickhouse-server`'s bundled `wget` HTTPS support — open deferral `d1`, needs a live pod
